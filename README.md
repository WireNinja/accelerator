...
Restore requires an exact backup ID and matching deployment key, stage, checksum, components, and active Git revision. It creates an emergency full backup before mutation, locks deployment, keeps a mode-`0600` local maintenance receipt until success, isolates only the selected stage's application writers, restores the supported local database and/or mutable files, then verifies HTTPS and services. It never changes code, runs migrations, restores across stages, or stops/restores NightOwl.

Deployed stages default to dual-destination backup: `local` plus Accelerator's private `accelerator-s3` disk. Configure `ACCELERATOR_BACKUP_S3_ACCESS_KEY_ID`, `ACCELERATOR_BACKUP_S3_SECRET_ACCESS_KEY`, `ACCELERATOR_BACKUP_S3_BUCKET`, and `ACCELERATOR_BACKUP_S3_ENDPOINT` in each ignored stage env. Cloudflare R2 uses region `auto`; the default object layout is `accelerator/{deployment_key}/{stage}/acc-{deployment_key}-{stage}/...`, so one bucket remains collision-free across projects. Set `ACCELERATOR_BACKUP_S3_ENABLED=false` only when same-VPS-only durability is an explicit trade-off. The general Cloudflare API token is not used.
