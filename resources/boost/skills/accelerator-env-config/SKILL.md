...
Deployment reconfiguration preserves existing per-stage ports, workers, queues, service groups, and topology tuning. Replace ACME email only with explicit `--ssl-email`. Stage-scoped `--rotate-app-key` and `--rotate-reverb-credentials` are destructive credential rotations intended for first deployment or an explicit incident response; never rotate an established live stage implicitly.
