...
Each stage owns its backup enablement, deterministic daily time, Laravel filesystem destinations, age/storage limits, optional native archive password, dedicated S3-compatible credentials, and operator Telegram pair. Preserve these values during reconfiguration. `ACCELERATOR_BACKUP_S3_ENABLED=true` appends the private `accelerator-s3` destination to the ordinary disks and is the deployed-stage default; use `false` only when same-VPS backup is an accepted product trade-off. Never place `accelerator-s3` in `ACCELERATOR_BACKUP_DISKS` manually. `ACCELERATOR_TELEGRAM_*` is operational alerting; it is not user/profile `TELEGRAM_BOT_TOKEN`. See `../accelerator-deployment/references/backup-restore.md` before changing this contract.
