...
task('accelerator:backup-restore', function () use ($config): void {
    ...
    $maintenance = false;
    $phase = 'preflight';
    $cronFile = '/etc/cron.d/'.$config->cronName();
    $disabledCronFile = $cronFile.'.accelerator-restore';
    ...
    try {
        ...
        run('if command sudo -n test -f '.escapeshellarg($cronFile).'; then command sudo -n mv '.escapeshellarg($cronFile).' '.escapeshellarg($disabledCronFile).'; fi');
        run('cd '.escapeshellarg($config->currentPath())
            .' && command sudo -n -u '.escapeshellarg($config->runUser)
            .' '.escapeshellarg($config->phpBinary).' artisan queue:pause default --no-interaction'
            .' && command sudo -n -u '.escapeshellarg($config->runUser)
            .' '.escapeshellarg($config->phpBinary).' artisan schedule:interrupt --no-interaction');
        run('deadline=$((SECONDS + 135)); while true; do active=no; '
            .'for pid in $(pgrep -u '.escapeshellarg($config->runUser).' -f '.escapeshellarg('artisan queue:work database --queue=default').' 2>/dev/null || true); do '
            .'if [ "$(readlink -f /proc/$pid/cwd 2>/dev/null || true)" = '.escapeshellarg($config->currentPath()).' ]; then active=yes; break; fi; done; '
            .'if [ "$active" = no ]; then break; fi; if [ $SECONDS -ge $deadline ]; then echo "Timed out waiting for the stage queue drain." >&2; exit 1; fi; sleep 1; done');
        ...
        run('cd '.escapeshellarg($config->currentPath())
            ...
            .' && command sudo -n -u '.escapeshellarg($config->runUser)
            .' '.escapeshellarg($config->phpBinary).' artisan queue:resume default --no-interaction'
            .' && command sudo -n -u '.escapeshellarg($config->runUser)
            .' '.escapeshellarg($config->phpBinary).' artisan up --no-interaction');
        $maintenance = false;
        run('if command sudo -n test -f '.escapeshellarg($disabledCronFile).'; then command sudo -n mv '.escapeshellarg($disabledCronFile).' '.escapeshellarg($cronFile).'; fi');
        ...
        run('command sudo -n test -f '.escapeshellarg($cronFile));
        ...
        throw new RuntimeException(
            "Restore failed during {$phase}".($maintenance ? ' while the stage remains in maintenance mode with its scheduler cron disabled.' : ' before maintenance mode was enabled.').' '.$exception->getMessage(),
            previous: $exception,
        );
    } finally {
...
task('accelerator:preflight', function () use ($config, $nodeEnvironment): void {
    ...
    foreach (['nginx', 'certbot', 'systemctl'] as $command) {
    ...
    $cronOwner = trim(run(
        'if [ ! -e '.escapeshellarg($expectedCron).' ]; then echo absent; '
        .'elif sudo -n grep -Fxq '.escapeshellarg($cronMarker).' '.escapeshellarg($expectedCron).'; then echo owned; '
        .'else echo collision:unmanaged; fi',
    ));

    if ($cronOwner === 'collision:unmanaged') {
        throw new RuntimeException("Deployment preflight failed: cron file [{$expectedCron}] exists without the expected Accelerator ownership marker.");
    }

    run("printf 'ACCELERATOR_PREFLIGHT deployment_key={$config->deploymentKey}\\nACCELERATOR_PREFLIGHT stage={$config->stage}\\nACCELERATOR_PREFLIGHT host={$config->sshHost}\\nACCELERATOR_PREFLIGHT domain={$config->domain}\\nACCELERATOR_PREFLIGHT root={$config->deployRoot}\\nACCELERATOR_PREFLIGHT root_state={$rootState}\\nACCELERATOR_PREFLIGHT nginx_state={$nginxOwner}\\nACCELERATOR_PREFLIGHT cron_state={$cronOwner}\\n'", forceOutput: true);
});

task('accelerator:schedule-interrupt', function () use ($config): void {
    run('if [ -x '.escapeshellarg($config->currentPath().'/artisan').' ]; then cd '.escapeshellarg($config->currentPath())
        .' && command sudo -n -u '.escapeshellarg($config->runUser)
        .' '.escapeshellarg($config->phpBinary).' artisan schedule:interrupt --no-interaction || true; fi');
});
...
task('accelerator:provision', function () use ($config, $renderer): void {
    ...
    file_put_contents($temporary.'/cron', $renderer->cron());
    ...
    upload($temporary.'/cron', '/tmp/'.$config->group.'-cron');
    run('command sudo -n install -m 0644 /tmp/'.$config->group.'-cron /etc/cron.d/'.$config->cronName());
    run('command sudo -n supervisorctl stop '.escapeshellarg($config->group.':*').' >/dev/null 2>&1 || true; command sudo -n rm -f '.escapeshellarg('/etc/supervisor/conf.d/'.$config->group.'.conf').'; command sudo -n supervisorctl reread >/dev/null 2>&1 || true; command sudo -n supervisorctl update >/dev/null 2>&1 || true');
...
task('accelerator:status', function () use ($config): void {
    ...
    run("if sudo -n test -f /etc/cron.d/{$config->cronName()}; then value=installed; else value=missing; fi; printf 'ACCELERATOR_STATUS scheduler_cron=%s\\n' \"\$value\"", forceOutput: true);
    run("printf 'ACCELERATOR_STATUS php_fpm=%s\\n' \"\$(sudo -n systemctl is-active {$config->fpmService} 2>/dev/null || true)\"", forceOutput: true);
...
before('deploy:symlink', 'accelerator:schedule-interrupt');
